  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (registration?.registrationId) {
        const result = await GenerationOrchestrator.generateBoth(registration.registrationId, false);
        if (result.success && result.certificateUrl && result.idCardUrl) {
          const baseName = name.replace(/\s+/g, "_");
          setDownloads([
            { url: result.certificateUrl, label: "Certificate", filename: `${baseName}_Certificate.jpg` },
            { url: result.idCardUrl, label: "ID Card", filename: `${baseName}_ID_Card.jpg` },
          ]);
          setModalOpen(true);
        } else {
          console.warn("Failed to generate certificate/ID card:", result.error);
        }
      }
    } catch (error) {
      setMessage({
...
return createPortal(
  <div className="fixed inset-0 flex justify-center items-center  z-[9999]">
    <div
      ref={modalRef}
      className={`
        w-[90vw] max-w-[1200px] min-h-[33vh] bg-gray-900 rounded-xl p-8 overflow-auto
        flex flex-col gap-6
      `}
      style={{
      ...
      <div className="w-full max-w-md bg-white p-6 rounded shadow">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="tel" required value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} className="w-full border p-2 rounded" />
          </div>

          <div>
            <label className="block mb-1">Organization *</label>
            <input type="text" required value={parlor} onChange={(e) => setParlor(e.target.value)} className="w-full border p-2 rounded" />
          </div>
          ...
          <div>
            ...
            <input type="text" required value={city} onChange={(e) => setCity(e.target.value)} className="w-full border p-2 rounded" />
          </div>

          <div>
            <label className="block mb-1">Upload Photo <span className="font-black text-red-600">POTRAIT with CLEAR FACE</span></label>
            <input type="file" accept="image/*" onChange={(e) => setPhoto(e.target.files?.[0] || null)} className="w-full border p-2 rounded" />
          </div>

          <div className="session-select">
            <label className="block mb-1">Select Session *</label>
            <select required value={sessionId} onChange={(e) => setSessionId(e.target.value)} className="w-full border p-2 rounded">
          ...
          <button type="submit" disabled={isProcessing} className="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700 disabled:bg-gray-400">
